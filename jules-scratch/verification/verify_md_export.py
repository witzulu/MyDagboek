...
def run(playwright):
    ...
    try:
        ...
        expect(page.locator("#report-content-wrapper")).to_be_visible(timeout=10000)

        # 4. Assert: Check if the "Export to Markdown" button is visible and click it
        export_button = page.get_by_role("button", name="Export to Markdown")
        expect(export_button).to_be_visible()
        export_button.click()

        # Give a moment for any potential errors to occur after clicking
        time.sleep(2)
