...
exports.getDashboardReport = async (req, res, next) => {
  try {
    ...
    // Global Stats
    const totalTasksCreated = await Task.countDocuments(baseQuery);
    const totalTasksCompleted = await Task.countDocuments({ ...baseQuery, completedAt: { $ne: null } });
    const totalTasksOverdue = await Task.countDocuments({ ...baseQuery, dueDate: { $lt: new Date() }, completedAt: null });
    const totalTasksInProgress = await Task.countDocuments({ ...baseQuery, list: { $nin: optionalListIds }, completedAt: null });

    // Per-Project Stats
    const projectsData = await Promise.all(userProjects.map(async (project) => {
      const projectBoards = await Board.find({ project: project._id });
      const projectBoardIds = projectBoards.map(b => b._id);
      const projectBaseQuery = { board: { $in: projectBoardIds } };

      const completedCount = await Task.countDocuments({ ...projectBaseQuery, completedAt: { $ne: null } });
      const totalCount = await Task.countDocuments(projectBaseQuery);
