...
const ProgressReports = () => {
  ...
  const handleExportMarkdown = () => {
    ...
    if (report.changelogEntries && report.changelogEntries.length > 0) {
        ...
        report.changelogEntries.forEach(entry => {
            ...
            mdContent += `**[${date}]** - **${userDisplay}** (${entry.type})\n\n`;
            if (entry.title) {
                mdContent += `### ${entry.title}\n\n`;
            }
            mdContent += `${entry.message}\n\n`;
            mdContent += `***\n\n`;
        });
    }
  ...
  const handleExportPDF = async () => {
    ...
    if (report && report.changelogEntries && report.changelogEntries.length > 0) {
        ...
        const automaticEntries = report.changelogEntries.filter(e => e.type === 'automatic');
        const manualEntries = report.changelogEntries.filter(e => e.type === 'manual');
        ...
        manualEntries.forEach(entry => {
            ...
            pdf.text(`[${date}] ${userDisplay} - ${entry.title}`, margin, yPos);
            yPos += 7;
            pdf.setFont(undefined, 'normal');
            const splitMessage = pdf.splitTextToSize(entry.message, pageWidth - margin * 2);
        ...
        if(automaticEntries.length > 0) {
            ...
            pdf.setFontSize(14);
            pdf.text('Automatic Log', margin, yPos);
            yPos += 8;
            pdf.autoTable({
                startY: yPos,
                head: [['Date', 'User', 'Change']],
                body: automaticEntries.map(entry => [
        ...
        <div className="p-6  w-full">
            ...
            <div className="flex items-center space-x-4 mb-6 p-4 bg-base-200 rounded-lg">
                <div>
                    ...
                    <input type="date" id="start-date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="mt-1 block w-full p-2 rounded border bg-base-100"/>
                </div>
                <div>
                    <label htmlFor="end-date" className="block text-sm font-medium">End Date</label>
                    <input type="date" id="end-date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="mt-1 block w-full p-2 rounded border bg-base-100"/>
                </div>
                <button onClick={handleGenerateReport} disabled={loading} className="self-end px-4 py-2 rounded-md bg-primary text-primary-content disabled:opacity-50">
                    {loading ? 'Generating...' : 'Generate Report'}
                </button>
                {report && (
                    <>
                        <button onClick={handleExportCSV} className="self-end px-4 py-2 rounded-md bg-info text-info-content">Export to CSV</button>
                        <button onClick={handleExportPDF} disabled={isExporting} className="self-end px-4 py-2 rounded-md bg-secondary text-secondary-content disabled:opacity-50">
                            {isExporting ? 'Exporting...' : 'Export to PDF'}
                        </button>
                        <button onClick={handleExportMarkdown} className="self-end px-4 py-2 rounded-md bg-accent text-accent-content">Export to Markdown</button>
                    </>
                )}
            </div>
